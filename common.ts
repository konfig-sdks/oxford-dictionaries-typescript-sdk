...
import { readableStreamToString, OxfordDictionariesError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new OxfordDictionariesError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new OxfordDictionariesError(e, e.response?.data)
                }
                if (innerError instanceof OxfordDictionariesError) {
                    // Got "OxfordDictionariesError" from the above try block
                    throw innerError;
                }
